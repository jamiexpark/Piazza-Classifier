...
public:
  ...
  //       using "Value_type()".
    Iterator find(const Key_type& k) const{
        ...
        return searchTree.find(Pair_type(k, Value_type()));
    }
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k){
        ...
        Iterator valType = find(k);
        if(valType != searchTree.end()){
            return valType->second;
        }else{
            return searchTree.insert(Pair_type(k, Value_type()))->second;
        }
    }
  ...
  //           the value true.
    std::pair<Iterator, bool> insert(const Pair_type &val){
        Iterator inserter = find(val.first);
        if(inserter != searchTree.end()){

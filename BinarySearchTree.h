...
public:
    ...
    bool check_tree_nodes(const Node *root){
        if(root->left == nullptr && root->right == nullptr){
            ...
            return check_tree_nodes(root->left) + check_tree_nodes(root->right);
  ...
  size_t height() const {
    ...
    int greatestPath() const{
        return greatest_path(root);
    }
...
//
//
        p->left = copy_nodes_impl(node->left);
          p->right = copy_nodes_impl(node->right);
          return p;
//
//      }
//      retu
...
//      }
    ...
    int greatest_path(const Node *root) const{
        int sum = 0;
        if(!root){
            return sum;
        }

        if(max(root->left->datum, root->right->datum)){
            sum = sum + root->left->datum;
            root = root->left;
            return greatest_path(root);
        }else if(max(root->right->datum, root->left->datum)){
            sum = sum + root->right->datum;
            root = root->right; ;
            return greatest_path(root);
        }
        return sum;
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
      ...
      if(!node){
          ...
          //if its empty
          if(empty_impl(node) == true && (node == nullptr)){
              return node;
          }
...
//          min_greater_than_impl(node->right, val, less);
//      }else{
//          Node *leftCheck = min_greater_than_impl(node->left, val, less);
//          if(leftCheck == nullptr){
//
//          }
//      }
//  }
